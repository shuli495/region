...
数据库连接和管理令牌只通过环境变量注入，不要写入 `config/`、PM2 配置、Dockerfile 或 GitHub Actions 文件。可用变量见 `.env.example`。
...
建议为应用创建仅能访问 `region` 库的独立数据库账号，不要使用 `root`。生产数据库应使用私有网络或 IP 白名单，并启用云数据库支持的传输加密。
...
REGION_VERSION_CHECK_ENABLED=false // 是否检查远程版本并处理增量，默认关闭
REGION_VERSION_AUTO_UPDATE=false // 是否自动切换行政区划数据版本，默认关闭
REGION_ADMIN_TOKEN= // POST /version 的 Bearer Token；为空时接口关闭

restart_command: 'yarn pm2:pro'  // 重启命令
```

启用版本检查会拉取代码并执行数据库增量 SQL，只应在受控发布流程中开启。管理令牌至少需要 32 个字符，可通过 `openssl rand -hex 32` 生成。调用版本管理接口时使用 `Authorization: Bearer <REGION_ADMIN_TOKEN>`。
